...
parser.add_argument('--emit-corrected', '-c', default=None, type=FileType('w'))
...
corrected_sequence = {rec.id: MutableSeq(str(rec.seq)) for rec in seq_recs}
...
for column in joint_vars.columns:
    ...
    raise ValueError("Could not find target and reference in "
                     +str(joint_vars.columns))
...
if args.threads == 1:
    ...
    for i, var in joint_vars.iterrows():
        ...
        if var['HOM-VAR'] == var['NCALLED'] and len(var['ALT']) == 1:
            ...
            corrected_sequence[var.CHROM][var.POS-1] = var['ALT']
            corrected_sites += 1
        elif var['HOM-REF'] != var['NCALLED']:
            if (args.emit_bed
                    and (var['HOM-REF'] == 1)
...
if args.emit_corrected:
    for rec in seq_recs:
        rec.seq = corrected_sequence[rec.id]
    SeqIO.write(seq_recs, args.emit_corrected, 'fasta')
